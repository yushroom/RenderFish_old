...
#pragma region BasicEffect
	...
	public:
		...
		void SetWorldViewProj(Matrix4x4 M)                  { WorldViewProj->SetMatrix(reinterpret_cast<const float*>(&M)); }
		void SetWorld(Matrix4x4 M)                          { World->SetMatrix(reinterpret_cast<const float*>(&M)); }
		void SetWorldInvTranspose(Matrix4x4 M)              { WorldInvTranspose->SetMatrix(reinterpret_cast<const float*>(&M)); }
		void SetTexTransform(Matrix4x4 M)                   { TexTransform->SetMatrix(reinterpret_cast<const float*>(&M)); }
		void SetEyePosW(const Vector3& v)                  { EyePosW->SetRawValue(&v, 0, sizeof(XMFLOAT3)); }
